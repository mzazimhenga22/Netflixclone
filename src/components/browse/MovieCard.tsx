...
export default function MovieCard({ movie }: MovieCardProps) {
  ...
  const computePosition = () => {
    ...
    if (top < 20) {
      top = 20;
    } else if (top + overlayHeight > window.innerHeight - 20) {
    ...
    if (left < 20) {
      left = 20;
    } else if (left + overlayWidth > window.innerWidth - 20) {
        ...
        <AnimatePresence>
            {showPreview && position && createPortal(
              <motion.div
                ...
                onMouseEnter={handleMouseEnter} // Keep preview open when mouse enters it
                onMouseLeave={handleMouseLeave}
                style={{
                  position: "fixed",
                  top: position.top,
